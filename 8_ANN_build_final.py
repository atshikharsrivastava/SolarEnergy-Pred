##################################################
#run from shell->  python 8_ANN_finalModels_eu.py station_number(1 to 16)
##################################################
...
if not os.path.exists("/home/shikhar/Europe/"+str(station_name)+"/ann/"):
	os.makedirs("/home/shikhar/Europe/"+str(station_name)+"/ann/")

os.chdir("/home/shikhar/Europe/"+str(station_name)+"/ann/")
...
os.chdir("/home/shikhar/Europe/"+str(station_name)+"/ann/"+directory_name)
...
if not os.path.exists(directory_name_cells):
	...
	os.chdir("/home/shikhar/Europe/"+str(station_name)+"/ann/"+directory_name+directory_name_cells)
...
def train_and_evaluate_model( train_data, train_target, trainDate,  val_localdata, val_localtarget, val_localDate, test_localdata, test_localtarget, test_localDate,  scaler ,directory_name_cells,time_Steps,neurons1,neurons2,epoch_global):
	...
	for iter in range(epochs):
		...
		valScore_mae = (sklearn.metrics.mean_absolute_error(y_val[:,0], valPredict[:,0]))
		...
		testScore_mae = (sklearn.metrics.mean_absolute_error(y_test_local[:,0], test_localPredict[:,0]))
		...
		trainScore_mae = math.sqrt(sklearn.metrics.mean_squared_error(y_train[:,0], trainPredict[:,0]))
		
		df_tosave = pandas.DataFrame({'iterations' : [int(iter+1)], 'train_score' : [(trainScore)], 'val_score' : [(valScore)],'test_score' : [(test_localScore)], 'train_score_rel' : [(trainScore_mae)], 'val_score_rel' : [(valScore_mae)],'test_score_rel' : [(testScore_mae)]})
		...
		if  (iter+1) == 1:
			os.chdir("/home/shikhar/Europe/"+str(station_name)+"/ann/RMSE/")
		...
		valScore_focused_mae = (sklearn.metrics.mean_absolute_error(y_val_focused[:,0], valPredict_focused[:,0]))
		...
		testScore_focused_mae = (sklearn.metrics.mean_absolute_error(y_test_local_focused[:,0], test_localPredict_focused[:,0]))
		...
		trainScore_focused_mae = math.sqrt(sklearn.metrics.mean_squared_error(y_train_focused[:,0], trainPredict_focused[:,0]))
		
		df_tosave2 = pandas.DataFrame({'iterations' : [int(iter+1)], 'train_score' : [(trainScore_focused)], 'val_score' : [(valScore_focused)],'test_score' : [(test_localScore_focused)], 'train_score_rel' : [(trainScore_focused_mae)], 'val_score_rel' : [(valScore_focused_mae)],'test_score_rel' : [(testScore_focused_mae)]})
		cols=['iterations', 'train_score', 'val_score','test_score', 'train_score_rel', 'val_score_rel','test_score_rel']
		...
		if iter+1 == 1 :
			...
			os.chdir("/home/shikhar/Europe/"+str(station_name)+"/ann/scores/")
